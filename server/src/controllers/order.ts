...
export const newOrder=Trycatch(
    async(req:Request<{},{},OrderRequestBody>,res,next)=>{
        const {shippingInfo,
            orderItems,
            shippingCharges,
            subtotal,
            ...
            if(!shippingInfo||
                !orderItems||
                !user || !total) return next(new ErrorHandler("Please fill all the feeds",400))
...
export const myOrders=Trycatch(async(req,res,next)=>{
    ...
    if(myCache.has(`myorder-${user}`)){
        orders=JSON.parse(myCache.get(`myorder-${user}`) as string)
    }
     else{
...
export const allOrders=Trycatch(async(req,res,next)=>{
    ...
    if(myCache.has("allorder")){
        orders=JSON.parse(myCache.get("allorder") as string)
    }
     else{
