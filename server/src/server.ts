...
server.use(cors({
    origin:process.env.CLIENT_ID as string,
...
import { NewOrderProps } from "./types/types.js";
...
server.post("/api/v1/genrate-order",Trycatch(async(req:Request<{},{},NewOrderProps>,res:Response,next:NextFunction)=>{

    console.log(req.body)
    const {user,userInfo,products,amount,orders}=req.body;

    const razorpayInstance = new Razorpay({
        key_id:process.env.KRY_ID as string,
    ...
    if(!order) return next(new ErrorHandler("Failed to genrate order",500)) 
...
server.post("/api/v1/payment-verification",async(req:Request,res:Response,next:NextFunction)=>{

    const {razorpay_payment_id, razorpay_order_id, razorpay_signature} = req.body

    const body = razorpay_order_id + "|" + razorpay_payment_id;

    const genrateSignature = crypto.createHmac("sha256",process.env.KEY_SECRET as string)
